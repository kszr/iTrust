...
Enumeration ben = session.getAttributeNames();
while(ben.hasMoreElements()){
	String o = ben.nextElement().toString();
	System.out.println(o);
	System.out.println(session.getAttribute(o));
} */
...
if (formIsFilled) {
	...
	personnelForm = new BeanBuilder<PersonnelBean>().build(request.getParameterMap(), new PersonnelBean());
	    SearchUsersAction action = new SearchUsersAction(prodDAO, loggedInMID.longValue());
	
	//List<PersonnelBean> personnel = action.fuzzySearchForExperts(pidString);
	List<PersonnelBean> personnel = action.searchForPersonnelWithName(personnelForm.getFirstName(), personnelForm.getLastName());
	//System.out.println(personnel.isEmpty());
	if(!personnel.isEmpty()){
	String specialty = personnel.get(0).getSpecialty();
	//System.out.println(specialty);
	personnelForm.setSpecialty(specialty);   
	}
    if(personnelForm.getStreetAddress1().length() > 20){
    	String[] s = personnelForm.getStreetAddress1().split(" ");
    	String s1 = "";
    	String s2 = personnelForm.getStreetAddress2();
    	int i;
    	for(i = 0; i < s.length; i++){
    		if(s1.length() + s[i].length() <= 20){
    			s1 = s1 + " " + s[i];
    		}
    		else 
    			break;
    	...
    	personnelForm.setStreetAddress1(s1);
    	if(personnelForm.getStreetAddress2().length() <= 20)
    	personnelForm.setStreetAddress2(s2);
    	else
    		out.println("<span class=\"font_failure\">" + "Street Address line 2 too long!" + "<br>");	
