...
public class FindExpertActionTest extends TestCase {
	...
	public void testFindExpertsForLocalHospitals(){
		List<HospitalBean> hospitals = new ArrayList<HospitalBean>();
		HospitalBean realHospital = new HospitalBean("1");
		hospitals.add(realHospital);
		System.out.println(fea.findExpertsForLocalHospitals(hospitals, "all").size());
		assertTrue(fea.findExpertsForLocalHospitals(hospitals, "all").size() != 0);
	}
}
